...
from bots.aspects.autodelete import AutoDeleteStorage, install_autodelete
    ...
    if not users_storage.is_active(update.effective_chat.id):
        auto_delete_storage.schedule(
            update.message.reply_text(
                "Ваш запрос на активацию бота должен быть одобрен администратором"
            )
        )
        return
                    ...
                    [
                        [
                            InlineKeyboardButton(
                                "\U0001f5d1\ufe0f Удалить",
                                callback_data=json.dumps({"type": "delete"}),
                            )
                        ]
                    ]
                ),
...
UsersBehavior(
    users_storage, auto_delete_storage, ADMIN_MSG_TTL, admin_greeting=admin_greeting
).install(dp)

dp.add_handler(MessageHandler(Filters.text, on_text))
dp.add_handler(CallbackQueryHandler(on_callback))

install_autodelete(dp, updater, auto_delete_storage, interval=3)
